...
- エントリの `scheduled` はGETレスポンスに存在するときだけマージし、欠落を `false` とみなしてPUTへ送らない。新規投稿の `scheduled=true` には `draft=true` と `updated` を必須とする。
- 新規固定ページの `updated` は任意指定でき、省略時の公開日時ははてな側に委ねる。
